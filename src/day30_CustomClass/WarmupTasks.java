...
public class WarmupTasks {
    public static void main(String[] args) {
        ...
        numbers.addAll(Arrays.asList(1,2,3,4,5,6,7,8)); //convert array to arraylist with Arrays.asList

        Collections.swap(numbers,0,numbers.size()-1);// first index number and last index number:size -1

        System.out.println(numbers);

       // int [] arr = {1,2,3,4,5,6,7,8};
       // System.out.println(Arrays.toString(arr)); how to print array
        ...
        int newSize = list.size(); // the size after removing 0

        int totalNumberOfZeros = size -newSize;

        System.out.println("totalNumberOfZeros = " + totalNumberOfZeros);

        for (int i = 0; i < totalNumberOfZeros; i++) { //add 0 back to arrayList
        ...
        letters.removeIf(p->Character.isLetter(p)); // remove the characters that are not letter
                                                     // lambda expression; p is each elements
